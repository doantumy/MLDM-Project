...
def freeman_chain_code(image_matrix, img_type='normal'):
    ...
    image_matrix = cv2.medianBlur(image_matrix, 3)
    
    # Thresholding the image
    _, image_matrix = cv2.threshold(image_matrix, t,
...
def get_image_from_file(path_to_image):
    ...
    image_matrix = cv2.imread(path_to_image, 0)
    # Switch black and white pixels values
    image_matrix = cv2.bitwise_not(image_matrix)
    
    return image_matrix

def freeman_from_file(path_to_image):
